 ...
 * 
 * This class wraps a base adapter that's passed into the constructor. It works
 * by creating extra view items types that are returned in
 * {@link #getItemViewType(int)}, and mapping these to the header and footer
 * views provided via {@link #addHeader(View)} and
 * {@link #addFooter(View)}.
 * 
 * There are two restrictions when using this class:
 * 
 * 1) The base adapter can't use negative view types, since this class uses
 * negative view types to keep track of header and footer views.
 * 
 * 2) You can't add more than 1000 headers or footers.
 * 
 * Created by mlapadula on 12/15/14.
 */
public class Bookends<T extends RecyclerView.Adapter> extends
	 ...
	 * 
	 * How this works: - Regular views use view types starting from 0, counting
	 * upwards - Header views use view types starting from -1000, counting
	 * upwards - Footer views use view types starting from -2000, counting
	 * upwards
	 * 
	 * This means that you're safe as long as the base adapter doesn't use
	 * negative view types, and as long as you have fewer than 1000 headers and
	 * footers
	 */
	...
	private boolean isHeader(int viewType) {
		return viewType >= HEADER_VIEW_TYPE
				&& viewType < (HEADER_VIEW_TYPE + mHeaders.size());
	}

	private boolean isFooter(int viewType) {
		return viewType >= FOOTER_VIEW_TYPE
				&& viewType < (FOOTER_VIEW_TYPE + mFooters.size());
	}
	...
	public int getItemViewType(int position) {
		if (position < mHeaders.size()) {
			...
			return FOOTER_VIEW_TYPE + position - mHeaders.size()
					- mBase.getItemCount();
		}
	}
}
